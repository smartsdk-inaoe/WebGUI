 ...
 * File: index.js
 *
 * This file manages the connection between the page and the kurento media server to play videos.
 *
...
ws.onmessage = function(message) {
	...
	case 'videoInfo':
		...
		document.getElementById('seekBar').min = parsedMessage.initSeekable;
		document.getElementById('seekBar').max = parsedMessage.endSeekable;
                document.getElementById('durationtag').innerHTML=Math.floor(parsedMessage.videoDuration/60000)+" minutes, "+(parsedMessage.videoDuration/1000)%60+" seconds";
		positionInterval = setInterval(getPosition,500);
	...
	case 'seek':
		...
		break;
	case 'position':
		document.getElementById("seekBar").value = parsedMessage.position;
		break;
	case 'iceCandidate':
		break;
	default:
